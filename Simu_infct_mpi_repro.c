...
int main(int argc, char *argv[])
{
    ...
    {
        // === FIX DE REPRODUCTIBILITÉ : INITIALISATION ===
        // Chaque agent reçoit exactement les mêmes coordonnées de départ et
        // durées d'incubation (dE, dI, dR), peu importe qui les calcule.
        srand(i * 2002 + 42);
    ...
    for (int i = 0; i < time; i++)
    {
        ...
        {
            // === FIX DE REPRODUCTIBILITÉ ===
            srand((j * 1000000) + (i * 10)); // Seed unique: Agent X au Jour Y
        ...
        for (int j = start; j < end; j++)
        {
            // === FIX DE REPRODUCTIBILITÉ DEUXIÈME PASSE ===
            srand((j * 1000000) + (i * 10) + 1); // Seed unique: Agent X au Jour Y passe 2
